# dbt_olist — Day 3 Solution (backbone + 3 business cases)

The example backbone **plus** the three business-case marts implemented the correct, best-practice
way (integrated into `models/`, so `dbt build` produces the full set and all tests pass):

- staging: `stg_sellers`, `stg_order_reviews`   ·   intermediate: `int_order_reviews`
- marts: `gld_seller_scorecard`, `gld_customer_cohorts`, `gld_review_sentiment_by_category`
- tests/docs distributed into the per-layer YAML (`models/staging/_stg_models.yml`, `models/intermediate/_int_models.yml`, `models/marts/_mart_models.yml`, each under a `# ---- business cases ----` marker) + the singular test `tests/assert_scorecard_revenue_non_negative.sql`

The exercise version (backbone only + stubs) is in `../../exercises/dbt_olist/`.

> **Data caveat (known, accepted simplification).** `order_reviews` is at **order grain** — it has
> no `order_item_id`/`seller_id`, and `review_id` is **not unique** (814 duplicate ids; an order can
> carry up to 3 reviews). Also ~1,278 orders (1.3%) contain items from **more than one seller**.
> So in `gld_seller_scorecard` the order-level **review score** and the item-weighted
> **avg_delivery_days** are attributed to *every* seller in a multi-seller order — i.e. seller-level
> review/delivery metrics are **approximate**. We accept this for the exercise rather than
> introducing an item↔review allocation rule the source data can't support.
>
> **Cohorts (`gld_customer_cohorts`).** Repeat buying is rare in Olist (~3% of customers order
> again), and the earliest cohorts are tiny (2016‑09 has 4 customers, 2016‑12 has 1) → their
> `repeat_rate_pct` is statistical noise (e.g. 100% off a single customer). Treat small/early
> cohorts with caution. `repeat_rate`/`avg_lifetime_value` also count **all order statuses**
> (the dim is built on every order, not delivered‑only) — fine here; filter to delivered if you
> want pure revenue cohorts.
>
> **Review sentiment (`gld_review_sentiment_by_category`).** A review is counted **once per
> category of its order** (deduped at category level). Only ~0.8% of orders span >1 category, so
> the per‑category double‑count is negligible; an `'unknown'` category bucket holds products with
> no category name.
